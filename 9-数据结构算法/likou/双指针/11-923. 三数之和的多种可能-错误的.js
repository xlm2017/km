...
var threeSumMulti = function (arr, target) {
  ...
  for (let i = 0; i < nums.length - 2; i++) {
    ...
    while (l < r) {
      if (nums[l] + nums[r] + nums[i] === target) {
        console.log(map.get(nums[l]) * map.get(nums[r]) * map.get(nums[i]))
  ...
  map.forEach((val, key) => {
    if (key === 0) {
      ...
      } else if (target !== 0 && map.get((target - key) / 2) >= 2) {
        ans = (ans + map.get((target - key) / 2) * (map.get((target - key) / 2) - 1) / 2 * val) % mod;
      }
    } else if (target / key === 3 && val >= 3) {
      ans = (ans + val * (val - 1) * (val - 2) / 6) % mod;
    } else if (map.get((target - key) / 2) >= 2) {
      ans = (ans + map.get((target - key) / 2) * (map.get((target - key) / 2) - 1) / 2 * val) % mod;
    }
  })
  return ans % mod;
