...
// 但是有时候二叉树的非递归我们不仅仅是为了解决遍历问题，
// 例如后序遍历堆栈里保存的始终是该节点的祖先节点这一性质可以用于解决寻找公共祖先的问题。
// 遇到这种问题作者的这种反转先序遍历的思路就不行了。
// 所以最好还是可以快速写出常规的非递归遍历比较好。
// 懒得写题解了，借作者宝地写一个本人写的三种非递归的统一模板式写法。 
// 二叉树的非递归难点其实就在于后序遍历，因为后序需要两次路过根节点。
// 遍历的时候需要根据第几次路过来决定是否访问根节点。
// 解决的办法是增加一个r指针，指向上次访问的节点。
// 因为后序遍历LRN最后一定是遍历根节点，所以当r指向root.right的时候那么就是该遍历root的时候了。
