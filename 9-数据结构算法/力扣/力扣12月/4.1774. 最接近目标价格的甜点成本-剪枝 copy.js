...
var closestCost = function (baseCosts, toppingCosts, target) {
  ...
  for (let i = 0; i < baseCosts.length; i++) {
    ...
    if (element < target) {
      ...
      // 最接近的值
      let nearest = element;
      function backtracking (array, index, deep) {
        ...
        if (sum < target) {
          ...
          res.push(stack1);
          // 比较前一个值
          console.log("大于:", sum, nearest);
          // if (Math.abs(sum - target) < Math.abs(target - (sum - stack[stack.length - 1]))) {
          //   console.log("改:", sum, sum - stack[stack.length - 1]);
          //   nearest = sum;
          // }
        ...
        for (let j = index; j < array.length; j++) {
          ...
          // 剪枝错误
          // if (deep % 2 === 1) {
          //   backtracking(array, j + 1, deep + 1);
          // } else {
          //   backtracking(array, j, deep + 1);
          // }
      ...
      console.log("主料小于目标值:", res, nearest);
