...
FreqStack.prototype.pop = function () {
  ...
  // console.log('统计结果:', map);

  // 次数最高的下标列表
  let times = 1
  let timesArr = []
  map.forEach((val, key, maps) => {
    // ...//操作
    // console.log(val, key, maps);
    if (val === times) {
      timesArr.push(key);
    } else if (val > times) {
      times = val;
  ...
  let indexs = timesArr.map((item, index) => {
    return this.arr.lastIndexOf(item);
  })
  maxTimesIndex = Math.max(...indexs);
