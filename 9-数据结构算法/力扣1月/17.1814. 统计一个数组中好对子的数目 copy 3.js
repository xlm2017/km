...
var countNicePairs = function(nums) {
  ...
  for (let i = 0; i < notOk.length; i++) {
    if(map.get(notOk[i]) > 1){
      // console.log("合格的map > 1", map.get(notOk[i]));
      // ans = ans + math(map.get(notOk[i]))/math(3)/2;
      ans = ans + map.get(notOk[i]) * (map.get(notOk[i]) - 1)/2;
    }
    for (let j = i + 1; j < notOk.length; j++) {
      if(notOk[i] + arr[j] === notOk[j] + arr[i]){
        // console.log("合格的:", notOk[i], notOk[j]);
        // ans++
        // console.log("合格的map:", map.get(notOk[i]), map.get(notOk[j]));
        ans = ans + map.get(notOk[i]) * map.get(notOk[j]);
      }      
    }    
  }
